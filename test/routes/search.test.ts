...
describe("Search routes", () => {
  ...
  describe("POST /api/search-streams", () => {
    ...
    it("returns 503 when no plugin is installed", async () => {
      const res = await fetch(`${baseUrl}/api/search-streams`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const body = await res.json() as { error: string };
      assert.equal(body.error, "no_source");
    });
  });
  ...
  describe("with a mock plugin installed", () => {
    let baseUrl2: string, close2: () => Promise<void>;
    before(async () => {
      const registry = new MockPluginRegistry({ installed: true, running: true });
      ({ baseUrl: baseUrl2, close: close2 } = await startTestServer({
        pluginRegistry: registry as unknown as PluginRegistry,
      }));
    });
    after(async () => { await close2(); });

    it("returns 200 with results from the plugin", async () => {
      const res = await fetch(`${baseUrl2}/api/search-streams`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: "Test Movie", year: 2020, type: "movie" }),
      });
    ...
    it("check-availability returns available items from the plugin", async () => {
      const res = await fetch(`${baseUrl2}/api/check-availability`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items: [{ id: 1, title: "Movie A", type: "movie" }] }),
      });
      assert.equal(res.status, 200);
      const body = await res.json() as { available: number[] };
      assert.ok(body.available.includes(1));
    });
  });
});
